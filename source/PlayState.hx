...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		var songLowercase = StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase();
			switch (songLowercase) {
				...
				case 'far-believes': songLowercase = 'farbelieves';
			}
			...
			case 'renstage':
				{
					...
					renbg = new FlxSprite(-240, -280);
					renbg.frames = Paths.getSparrowAtlas('renstage/stage','shared');
					renbg.animation.addByPrefix('idle', 'BG1', 24, true);
					renbg.antialiasing = true;
					renbg.scrollFactor.set(1, 1);
					renbg.setGraphicSize(Std.int(renbg.width * 1));
					renbg.updateHitbox();
					add(renbg);
					renbg.animation.play('idle');
				}
			default:
			{
					defaultCamZoom = 0.9;
		...
		if (gfVersion == 'gf-mayu' && curStage == 'renstage')
		{
			gf.x = 530;
			gf.y = 320;
		}
		...
		if (FlxG.save.data.songPosition) // I dont wanna talk about this code :(
			{
				...
				if (FlxG.save.data.downscroll)
					songName.y -= 3;
				songName.setFormat(Paths.font("pm.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 75,0,SONG.song + " " + (storyDifficulty == 2 ? "Hard" : storyDifficulty == 1 ? "Normal" : "Easy") + (Main.watermarks ? " - KE " + MainMenuState.kadeEngineVer : ""), 16);
		kadeEngineWatermark.setFormat(Paths.font("pm.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("pm.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		replayTxt.setFormat(Paths.font("pm.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		botPlayState.setFormat(Paths.font("pm.ttf"), 42, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
	...
	override public function update(elapsed:Float)
	{
		...
		#if debug
		//basic position editing shit my beloved
		if (FlxG.keys.pressed.SHIFT)
			mm = 10;
		else
			mm = 1;
		if (curStage == 'renstage')	
		{
	...
	function endSong():Void
	{
		...
		if (SONG.validScore)
		{
			...
			switch (songHighscore) {
				...
				case 'Far-Believes': songHighscore = 'Farbelieves';
			}
				...
				else
				{
					...
					var nextSongLowercase = StringTools.replace(PlayState.storyPlaylist[0], " ", "-").toLowerCase();
						switch (nextSongLowercase) {
							...
							case 'far-believes': nextSongLowercase = 'farbelieves';
						}
		...
		private function keyShit():Void // I've invested in emma stocks
			{
				...
				playerStrums.forEach(function(spr:FlxSprite)
				{
					if (pressArray[spr.ID] && spr.animation.curAnim.name != 'confirm')
						...
						if (!FlxG.save.data.botplay && !holdArray[spr.ID])
							spr.animation.play('static');
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curBeat % 2 == 0 && !boyfriend.animation.curAnim.name.startsWith("sing"))
		{
			boyfriend.playAnim('idle');
		}
